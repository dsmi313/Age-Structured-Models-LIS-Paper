    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Population Structure",
                 ...
                 helpText("Ages are tracked directly using cohort dynamics (age × length matrix). No inference required.",
                          tags$br(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$species, {
    if (input$species == "white_crappie") {
      ...
      updateNumericInput(session, "linf", value = 683)  # Craig et al. 1995
      updateNumericInput(session, "vbk", value = 0.32)  # Craig et al. 1995
      updateNumericInput(session, "t0", value = -0.52)  # Craig et al. 1995
      ...
      updateNumericInput(session, "linf", value = 584)  # Average of male/female
      updateNumericInput(session, "vbk", value = 0.22)  # Average of male/female
      ...
      updateNumericInput(session, "capsize", value = 250)
      ...
      updateNumericInput(session, "harvlim", value = 305)  # 12 inches minimum harvest size
      updateNumericInput(session, "linf", value = 592)  # Moderate growth
      updateNumericInput(session, "vbk", value = 0.17)
      ...
      updateNumericInput(session, "harvlim", value = 305)  # 12 inches minimum harvest size
      updateNumericInput(session, "linf", value = 1300)  # Moderate growth (larger species)
      ...
      updateNumericInput(session, "capsize", value = 350)
  ...
  build_length_bins <- function(input, bin_width = 10, growth_params = get_growth_params()) {
  ...
  build_recruit_distribution <- function(input, length_bins, growth_params = get_growth_params(), growth_cv_effective) {
  ...
  build_growth_matrix <- function(input, length_bins, growth_params = get_growth_params(), growth_cv_effective) {
    make_growth_matrix(
  ...
  build_static_components <- function(input) {
  ...
  simulate_population <- function(U, static, params) {
    ...
    if (store_details) {
      ...
      all_AgeAbund <- matrix(NA_real_, Amax, nsim)
    }

    for(k in 1:nsim) {
      if (!is.null(progress_cb)) {
        progress_cb(1/nsim, detail = paste("Simulation", k, "of", nsim))
      }
      ...
      Cohort <- matrix(0, Amax, L_bins)
      Cohort[1, ] <- Ro * recruit_dist
      N[1, ] <- colSums(Cohort)

      SSB_burnin <- rep(NA_real_, burn_in_years)
      ...
      h <- ifelse(isTRUE(params$enable_ddr), params$steepness, 0.7)
      SSB0 <- SPR_denom
      ...
      for(i in start_year:Ymax) {
        ...
        if(isTRUE(params$enable_ddr)) {
          if(i == start_year) {
            Rcapacity[i] <- Rcapacity[i-1]
          } else {
            SSB_t <- sum(N[i-1, ] * Fec_bins, na.rm = TRUE)
            ...
            if(isTRUE(params$enable_depensation) && SSB_t < 0.2 * SSB0) {
      ...
      harvest_window[, bin_midpoints < Harvlim] <- 0
      ...
      results$Recruit[k] <- mean(Rcapacity[last_50_start:Ymax], na.rm = TRUE)
  ...
  observeEvent(input$run_sim, {
    ...
    withProgress(message = 'Running simulation...', value = 0, {
      static <- build_static_components(input)

      params <- list(
        Amax = input$amax,
        burn_in_years = input$amax + 20,
        Ymax = (input$amax + 20) + 100,
        nsim = input$nsim,
        ...
        Harvlim = input$harvlim,
        store_details = TRUE,
        progress = function(value, detail = NULL) incProgress(value, detail = detail)
      )

      sim_out <- simulate_population(input$exploitation, static, params)
  ...
  # Population structure plot - TRUE AGE from cohorts
  output$pop_structure <- renderPlotly({
    req(pop_structure_data())
    pop_list <- pop_structure_data()
    age_data <- pop_list$age_data
    ...
    p <- ggplot(age_data, aes(x = Age)) +
      ...
      labs(title = "True Age Distribution at Equilibrium",
           subtitle = "Bars show mean; ribbon shows 95% PI. Ages tracked directly via cohort matrix.",
  ...
  output$vulnerability_plot <- renderPlotly({
    req(pop_structure_data())
    pop_list <- pop_structure_data()
    pop_data <- pop_list$length_data   # <- this is the data frame you want
  ...
  output$length_frequency <- renderPlotly({
    req(pop_structure_data())
    pop_list <- pop_structure_data()
    pop_data <- pop_list$length_data   # <- use the length data
  ...
  observeEvent(input$run_yield_curve, {
    withProgress(message = 'Generating yield curve...', value = 0, {
      ...
      curve_results <- simulate_yield_curve(static, params)
